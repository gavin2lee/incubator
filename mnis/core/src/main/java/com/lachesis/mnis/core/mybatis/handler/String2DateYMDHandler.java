...
public class String2DateYMDHandler implements TypeHandler<String> {

	@Override
	public String getResult(ResultSet rs, String fld) throws SQLException {
		return DateUtil.format(rs.getTimestamp(fld), DateFormat.YMD);
	}

	@Override
	public String getResult(ResultSet rs, int fldIdx) throws SQLException {
		return DateUtil.format(rs.getTimestamp(fldIdx), DateFormat.YMD);
	}

	@Override
	public String getResult(CallableStatement callable, int fldIdx)
			throws SQLException {
		return DateUtil.format(callable.getTimestamp(fldIdx), DateFormat.YMD);
	}
