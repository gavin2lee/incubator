...
public class DateToStringYMDHMHandler implements TypeHandler<String> {
	
	@Override
	public String getResult(ResultSet rs, String fld) throws SQLException {
		return DateUtil.format(rs.getTimestamp(fld).getTime(), DateFormat.YMDHM);
	}

	@Override
	public String getResult(ResultSet rs, int fldIdx) throws SQLException {
		return DateUtil.format(rs.getTimestamp(fldIdx), DateFormat.YMDHM);
	}

	@Override
	public String getResult(CallableStatement callable, int fldIdx) throws SQLException {
		return DateUtil.format(callable.getTimestamp(fldIdx), DateFormat.YMDHM);
	}
	...
	public void setParameter(PreparedStatement pstat, int fldIdx, String value,
		...
		if (value != null && !"".equals(value)) {
			pstat.setTimestamp(fldIdx, new Timestamp(DateUtil.parse(value, DateFormat.YMDHM).getTime()));
		}
	}
}
