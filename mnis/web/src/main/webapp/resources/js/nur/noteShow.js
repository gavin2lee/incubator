...
var note = {
  ...
  timers: (function() {
    var config = $.parseJSON(sessionStorage.getItem("config"))['data'],
      whiteBoardRefreshTime = config.system['whiteBoardRefreshTime'] && config.system['whiteBoardRefreshTime'].split(',');
    return {
  ...
  setSize: function() {
    ...
    $noScroll.each(function(i, normalItem) {
      ...
      $td.each(function(tdIndex, tdItem) {
        ...
        /*$title.css({
            'padding': Math.abs(($tdItem.height() - $title.height())) / 2 + 'px' + ' 6px'
        });*/
