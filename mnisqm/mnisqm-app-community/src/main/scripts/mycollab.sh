...
# -----------------------------------------------------------------------------
# Control Script for the mnisqm Server
#
# Environment Variable Prerequisites
#
#   MYCOLLAB_HOME   May point at your mnisqm "build" directory.
...
#                   only be used by mnisqm itself, not by the stop process,
...
if [ -z "$MYCOLLAB_OUT" ] ; then
  MYCOLLAB_OUT="$MYCOLLAB_HOME"/logs/mnisqm.out
fi
    ...
    if [ -f "$MYCOLLAB_PID" ]; then
      if [ -s "$MYCOLLAB_PID" ]; then
        ...
        if [ -r "$MYCOLLAB_PID" ]; then
          ...
          if [ $? -eq 0 ] ; then
            echo "mnisqm appears to still be running with PID $PID. Start aborted."
            exit 1
          else
 ...
 ####>> "$MYCOLLAB_OUT" 2>&1 "&"
  ...
  if [ ! -z "$MYCOLLAB_PID" ]; then
    if [ -f "$MYCOLLAB_PID" ]; then
      ...
      echo "\$MYCOLLAB_PID was set but the specified file does not exist. Is mnisqm running? Stop aborted."
      exit 1
    fi
  fi
  ...
  if [ ! -z "$MYCOLLAB_PID" ]; then
    if [ -f "$MYCOLLAB_PID" ]; then
      while [ $SLEEP -ge 0 ]; do
        ...
        if [ $? -gt 0 ]; then
          ...
          if [ $? != 0 ]; then
            if [ -w "$MYCOLLAB_PID" ]; then
              ...
              echo "mnisqm stopped but the PID file could not be removed or cleared."
            fi
          fi
          break
        fi
        ...
        if [ $SLEEP -eq 0 ]; then
          if [ $FORCE -eq 0 ]; then
            echo "mnisqm did not stop in time. PID file was not removed."
          fi
        fi
        SLEEP=`expr $SLEEP - 1 `
      done
  ...
  if [ $FORCE -eq 1 ]; then
    if [ -z "$MYCOLLAB_PID" ]; then
      ...
      if [ -f "$MYCOLLAB_PID" ]; then
        PID=`cat "$MYCOLLAB_PID"`
        echo "Killing mnisqm with the PID: $PID"
        kill -9 $PID
        while [ $KILL_SLEEP_INTERVAL -ge 0 ]; do
            ...
            if [ $? -gt 0 ]; then
                rm -f "$MYCOLLAB_PID" >/dev/null 2>&1
                if [ $? != 0 ]; then
                    echo "mnisqm was killed but the PID file could not be removed."
                fi
                break
            fi
        ...
        if [ $KILL_SLEEP_INTERVAL -gt 0 ]; then
            echo "mnisqm has not been killed completely yet. The process might be waiting on some system call or might be UNINTERRUPTIBLE."
        fi
      fi
    fi
  fi
  ...
  echo "Usage: mnisqm.sh ( commands ... )"
  echo "commands:"

  echo "  start             Start mnisqm in a separate window"
  echo "  stop              Stop mnisqm, waiting up to 5 seconds for the process to end"
  echo "  stop -force       Stop mnisqm, wait up to 5 seconds and then use kill -KILL if still running"
