(function() {
  ...
  angular
    .module('nqms')
    .directive('acmeBreadcrumb', acmeBreadFunc);

  /** @ngInject */
  function acmeBreadFunc($interpolate, $state) {
    return {
      restrict: 'E',
      templateUrl: function(elem, attrs) {
        var templateUrl = 'app/components/breadcrumb/breadcrumb.tpl.html';
      ...
      controller: function ($scope) {
        ...
        监听菜单模式
         */
        $scope.$on('toggleNavView', function(event, isMini) {

          $scope.minMode = isMini;
        });

        $scope.$on('toggleSubNavView', function(event, isShow) {

          $scope.isSubNavShow = isShow;
        });
      },
      link: function(scope) {
        scope.breadcrumbs = [];
