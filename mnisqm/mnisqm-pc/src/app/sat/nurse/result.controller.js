(function() {
  ...
  angular
    .module('nqms')
    .controller('SatNurseResultController', SatNurseResultController);

  /** @ngInject */
  function SatNurseResultController($rootScope, $scope, $uibModal, $filter, NgTableParams, satNurseService, deptService) {
    var vm = this;
    ...
    function del(id) {
      satNurseService.deleteSatResult(id).then(function(response) {
    ...
    function open(data) {
      var modalInstance = $uibModal.open({
        templateUrl: 'app/sat/nurse/result.modal.html',
        controller: 'SatNurseResultModalController',
        controllerAs: 'vm',
        backdrop: 'static',
        animation: true,
        size: 'lg',
    ...
    function load() {
      satNurseService.getSatResult(vm.params).then(function(rsp) {
...
(function() {
  ...
  angular
    .module('nqms')
    .controller('SatNurseResultModalController', SatNurseResultModalController);

  /** @ngInject */
  function SatNurseResultModalController($rootScope, $scope, $uibModalInstance, NgTableParams, deptService, satNurseService, id) {
    var vm = this,
      handler;
    ...
    if (id) {
      vm.detail = id;
      handler = satNurseService.updateSatResult;
    } else {
      vm.detail = {
        satResultDetailList: [],
        userType: '1'
      ...
      handler = satNurseService.saveSatResult;
      vm.isNew = true;
    }
    ...
    function reload() {
      if (vm.tmpCode) {
        ...
        satNurseService.getSatTempByCode(vm.tmpCode)
          .then(function(rsp) {
