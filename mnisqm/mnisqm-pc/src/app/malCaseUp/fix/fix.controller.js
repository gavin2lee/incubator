(function() {
  ...
  function handleControllerFun($rootScope, $scope, $state, $log, $q, security, levelService, modalService, toastr, deptService, fixService, filesService) {
    var vm = this;
    var formData = {};
    var metaData = {};
    ...
    // 获取分级数据
    function initData() {
      return fixService.getEventList().then(function(response) {
        if (response) {
      ...
      modalService.open({
        ...
        methodsObj: {
          ...
          saveListItem: function(listName, itemData) {
            ...
            // 保存到服务器
            fixService['save' + list_name](data_temp).then(function(response) {
              if (response) {
          ...
          removeListItem: function(params) {
            ...
            fixService['delete' + list_name](_params).then(function(response) {
              if (response) {
    ...
    function initModalData(row) {
      ...
      var getEventRequirement = fixService.getEventRequirement({
        reportCode: reportCode
      });
      var getEventMeasures = fixService.getEventMeasures({
        reportCode: reportCode
      });
      var getReportDetails = fixService.getReportDetail({
        reportCode: reportCode
      });
      // var getEventRequirement = fixService.getEventRequirement({reportCode: reportCode});
    ...
    function saveReportDetail() {
      ...
      // return;
      fixService.saveReportDetail(data_arr).then(function(response) {
