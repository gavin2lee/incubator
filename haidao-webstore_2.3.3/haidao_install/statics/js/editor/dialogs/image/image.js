...
//                }
        ...
        callback: function (editor, $w, url, state, json) {
            if (json.status == 1) {
                //显示图片计数+1
                Upload.showCount++;
        ...
        queue: function(obj){
        	if($("#js_jqUi").length<1){
				...
				_script.src = "./statics/js/wap/jquery-ui.min.js";
				_script.id = "js_jqUi";
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="editor" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
        ...
        uploadComplete: function(r){
            ...
            try{
                var json = eval('('+r+')');
                Base.callback(me.editor, me.dialog, json.url, json.state, json);
            }catch (e){
        ...
        submit: function (callback) {
            ...
            var me = this,
                input = $( '<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus="" name="editor" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'),
                input = input[0];
