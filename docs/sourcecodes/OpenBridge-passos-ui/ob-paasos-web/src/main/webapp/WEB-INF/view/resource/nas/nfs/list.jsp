    ...
    <tbody>
		...
		<c:forEach items="${ pageData }" var="row"> 
		    <tr data-export='${row.allocateContent }'  class="nfsInfo" data ="${row.nasId }" id="${row.nasId }"  data-name="${row.instanceName }" data-envinfo='${row.requestEnvInfo }'  data-nastype="${row.nasType }">
				...
				<c:if test="${!fromdialog}">
			        <td>
			        	<a href="javascript:void(0);" onclick="deletePaasNas('${row.nasId}')">删除</a>&nbsp;&nbsp;
			        	<%--
			        	<c:choose>
...
<c:choose>
	...
	<c:otherwise>
		<c:if test="${!param.dialog }"><p class="no-content f14">你还没有任何实例，现在就<a href="/os/resource/nas/nfs/add.do">创建第一个</a>吧</p></c:if>
	</c:otherwise>
</c:choose>
<c:if test="${fromdialog}">
	<script>
