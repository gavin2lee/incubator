...
<template:include file="/common/template/dialog.jsp">
	<template:replace name="body">
		...
		<form action="${WEB_APP_PATH}/sys/access/role/save.do" method="post" id="addRoleForm">
		<div class="r_block">
	        <div class="r_con p10_0">
	            <div class="form_control p20">
	                <div class="form_block">
	                    <label>角色名称</label>
	                    <input name="roleSystem" value="paasos" type="hidden" />
	            ...
	            <%--     <div class="form_block">
	                    <label>包含人员</label>
	                    <input type="hidden" name="userIds" id="userIds" value="${role.userIds}" />
	                    <span id="userNames" style="border: 1px solid #eaeaea; width: 360px; height: 80px; vertical-align: top;overflow: auto;">
	                    	<c:forEach items="${ role.users }" var="user">
	                    	<span style="padding: 5px;">${ user['userName'] };</span>
	                    	</c:forEach>
						</span>
	                    <button onclick="selectUser()" type="button" style="border: 1px #eaeaea solid; width: 30px; height: 82px;margin-left: -3px;background-color:#eaeaea;cursor:pointer;"><i class="tit_ico edit_ico"></i></button>
	                </div>  --%>
	                <div class="form_block">
	...
	<template:replace name="bottom">
		...
		function onUserEnter(){
	    	...
	    	if(dataValidation()){
		    	...
		    	$.getJSON(url,params,function(json){ 
	    			loading.close();
		    		if(json.code==0){ 
		    			top.common.tips("添加成功",1,function(){
		...
		function dialogUser(multiple,valId,textId,cb,params){
			var dialog = top.common.dialogIframe("请选择用户",WEB_APP_PATH+"/sys/user/dialog.do?multiple="+multiple,650,600,null,['确定','取消'],{
