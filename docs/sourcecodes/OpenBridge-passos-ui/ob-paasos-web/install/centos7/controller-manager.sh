...
cat <<EOF >${kube_cfg}/kube-controller-manager.conf
...
KUBE_CONTROLLER_MANAGER_SERVICE_ACCOUNT_PRIVATE_KEY_FILE="--service-account-private-key-file=/etc/kube-serviceaccount.key"
EOF
...
[Service]
EnvironmentFile=-${kube_cfg}/kube-controller-manager.conf
ExecStart=${kube_bin}/kube-controller-manager ${KUBE_CONTROLLER_MANAGER_OPTS}
Restart=on-failure
