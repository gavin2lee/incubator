...
public abstract class AbstractSshMessage {
	...
	private final Log listener = LogFactory.getLog(AbstractSshMessage.class);
	 ...
	 *             on error
	 */
	protected Channel openExecChannel(final String command)
			throws JSchException {
	 ...
	 *             on other errors
	 */
	protected void waitForAck(final InputStream in) throws IOException,
			RuntimeException {
		final int b = in.read();
		...
		if (b == -1) {
			...
			if (b == 1) {
				throw new RuntimeException("server indicated an error: "
						+ sb.toString());
			} else if (b == 2) {
				throw new RuntimeException("server indicated a fatal error: "
						+ sb.toString());
			} else {
				throw new RuntimeException("unknown response, code " + b
						+ " message: " + sb.toString());
			}
		}
	}
	 ...
	 *             on ssh errors
	 */
	public abstract String execute() throws IOException, JSchException;
	 ...
	 *            the total length
	 */
	protected void logStats(final long timeStarted, final long timeEnded,
			final long totalLength) {
		final double duration = (timeEnded - timeStarted) / ONE_SECOND;
		final NumberFormat format = NumberFormat.getNumberInstance();
		format.setMaximumFractionDigits(2);
		format.setMinimumFractionDigits(1);
		listener.info("File transfer time: " + format.format(duration)
	 ...
	 */
	protected final int trackProgress(final long filesize,
			final long totalLength, final int percentTransmitted) {

		// CheckStyle:MagicNumber OFF
		final int percent = (int) Math.round(Math
